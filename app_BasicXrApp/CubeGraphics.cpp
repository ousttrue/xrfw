...
struct CubeGraphics : sample::IGraphicsPluginD3D11 {
  ...
  void InitializeD3DResources() {
    const winrt::com_ptr<ID3DBlob> vertexShaderBytes =
        sample::dx::CompileShader(CubeShader::ShaderHlsl, "MainVS", "vs_5_0");
    ...
    const winrt::com_ptr<ID3DBlob> pixelShaderBytes =
        sample::dx::CompileShader(CubeShader::ShaderHlsl, "MainPS", "ps_5_0");
  ...
  void Render(ID3D11Texture2D *colorTexture, int width, int height,
              const float projection[16], const float view[16]) override {}
  ...
  RenderView(const XrRect2Di &imageRect, const float renderTargetClearColor[4],
    ...
    // Create a DepthStencilView with the original swapchain format (swapchain
    // image is typeless)
    ...
    m_deviceContext->OMSetRenderTargets((UINT)std::size(renderTargets),
                                        renderTargets, depthStencilView.get());
    ...
    for (uint32_t k = 0; k < viewInstanceCount; k++) {
      const DirectX::XMMATRIX spaceToView =
          xr::math::LoadInvertedXrPose(viewProjections[k].Pose);
      const DirectX::XMMATRIX projectionMatrix = ComposeProjectionMatrix(
          viewProjections[k].Fov, viewProjections[k].NearFar);
    ...
    m_deviceContext->UpdateSubresource(m_viewProjectionCBuffer.get(), 0,
                                       nullptr, &viewProjectionCBufferData, 0,
                                       0);
    ...
    for (const sample::Cube *cube : cubes) {
      ...
      DirectX::XMStoreFloat4x4(
          ...
          DirectX::XMMatrixTranspose(
              scaleMatrix * xr::math::LoadXrPose(cube->PoseInAppSpace)));
      m_deviceContext->UpdateSubresource(m_modelCBuffer.get(), 0, nullptr,
                                         &model, 0, 0);

      // Draw the cube.
      m_deviceContext->DrawIndexedInstanced(
          (UINT)std::size(CubeShader::c_cubeIndices), viewInstanceCount, 0, 0,
          0);
    }
  }
