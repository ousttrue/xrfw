...
winrt::com_ptr<IDXGIAdapter1> GetAdapter(LUID adapterId) {
  ...
  CHECK_HRCMD(CreateDXGIFactory1(winrt::guid_of<IDXGIFactory1>(),
...
void CreateD3D11DeviceAndContext(
    IDXGIAdapter1 *adapter, const std::vector<D3D_FEATURE_LEVEL> &featureLevels,
...
winrt::com_ptr<ID3DBlob> CompileShader(const char *hlsl, const char *entrypoint,
